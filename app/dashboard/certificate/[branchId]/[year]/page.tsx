// app/dashboard/certificate/[branchId]/[year]/page.tsx
'use client'
...
export default function CertificateBatchPage({ params }: { params: Promise<{ branchId: string; year: string }> }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [branchRes, studentsRes] = await Promise.all([
        fetch('/api/branches'),
        fetch(`/api/students?branchId=${branchId}&year=${year}&level=CERTIFICATE`),
      ])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-slate-500 flex-wrap">
        ...
        <Link href="/dashboard/certificate" className="hover:text-slate-700">IT Certificate</Link><span>/</span>
        <Link href={`/dashboard/certificate/${branchId}`} className="hover:text-slate-700">{branch?.name || '...'}</Link><span>/</span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">IT Certificate · IMS Campus</p>
          </div>
        </div>
        <div className="sm:ml-auto flex gap-2">
          ...
          <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Students', value: students.length, bg: 'bg-blue-600', icon: Users },
          { label: 'Active Students', value: students.filter(s => s.isActive).length, bg: 'bg-emerald-600', icon: Users },
          { label: 'Payments Pending', value: 0, bg: 'bg-amber-500', icon: GraduationCap },
          { label: 'Exams Completed', value: 0, bg: 'bg-violet-600', icon: GraduationCap },
        ].map(s => (
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        ...
        {loading ? (
          <div className="py-12 text-center"><div className="w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto"></div></div>
        ) : filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <button onClick={() => setShowAddModal(true)} className="mt-4 flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium mx-auto">
              <Plus className="w-4 h-4" /> Add Student
            </button>
          </div>
        ) : (
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-blue-100 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
                          {student.photoPath ? <img src={student.photoPath} className="w-9 h-9 rounded-full object-cover" /> : <span className="text-blue-700 font-semibold text-sm">{student.fullName.charAt(0)}</span>}
                        </div>
      ...
      {showAddModal && (
        <AddStudentModal
          ...
          branchId={branchId} year={year} program="CERTIFICATE"
        />
      )}
    </div>
  )
...
export function AddStudentModal({ onClose, onSuccess, branchId, year, program }: {
  ...
  const otherProgram: 'CERTIFICATE' | 'DIPLOMA' = program === 'CERTIFICATE' ? 'DIPLOMA' : 'CERTIFICATE'
  const otherProgramLabel = program === 'CERTIFICATE' ? 'IT Diploma' : 'IT Certificate'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Add to primary program (normal add)
      await submitStudent(branchId, year, program, false)

      // Add to other program (cross add) if checkbox ticked
      if (addToBoth) {
        try {
          await submitStudent(otherBranchId, otherYear, otherProgram, true)
        } catch (err: any) {
          // Don't block — primary add succeeded
          console.warn('Cross-add failed:', err.message)
        }
      }
      onSuccess()
    } catch (err: any) {
  ...
  const programColor = program === 'CERTIFICATE' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-slate-700 hover:bg-slate-800'
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {[
            { label: 'Full Name (Certificate Name)', key: 'fullName', placeholder: 'Enter full name', type: 'text' },
            { label: 'Registration Number', key: 'regNumber', placeholder: 'e.g. IMS-GLL-2026-001', type: 'text' },
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading}
              className={`flex-1 px-4 py-2 ${programColor} text-white rounded-lg text-sm font-medium disabled:opacity-60 flex items-center justify-center gap-2`}>
              {loading
