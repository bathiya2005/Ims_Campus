// app/dashboard/diploma/[branchId]/page.tsx
'use client'
...
export default function DiplomaBranchPage({ params }: { params: Promise<{ branchId: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/students?branchId=${branchId}&level=DIPLOMA`).then(r => r.json()),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <Link href="/dashboard/diploma" className="hover:text-slate-700">IT Diploma</Link>
        <span>/</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="text-xs text-slate-500 uppercase tracking-wide">IT Diploma</div>
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-slate-700">{loading ? '...' : total}</div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {years.map(year => (
            <Link key={year} href={`/dashboard/diploma/${branchId}/${year}`}
              className="group bg-white rounded-xl border border-slate-200 p-6 hover:border-slate-400 hover:shadow-md transition-all text-center">
              <div className="w-12 h-12 bg-slate-100 group-hover:bg-slate-200 rounded-xl flex items-center justify-center mx-auto mb-3 transition-colors">
                <Calendar className="w-6 h-6 text-slate-500 group-hover:text-slate-700 transition-colors" />
              </div>
              <div className="text-2xl font-bold text-slate-800 group-hover:text-slate-900 transition-colors">{year}</div>
              ...
              <div className="mt-3 flex items-center justify-center gap-1.5">
                ...
                <span className="text-sm font-semibold text-slate-700">{yearCounts[year] || 0}</span>
